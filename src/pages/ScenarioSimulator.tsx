...
import { Play, RotateCcw, Thermometer, CloudRain, Users, Factory, TrendingUp, Flame, PartyPopper } from 'lucide-react';
            ...
            <div className="panel-elevated sticky top-24 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setParams({ ...params, forecastDays: Number(v) })}
                >
              ...
              <SliderInput
                label={t('populationIndex')}
                value={params.populationIndex}
                onChange={(v) => setParams({ ...params, populationIndex: v })}
                min={0.5}
                max={2.5}
                step={0.1}
              ...
              <SliderInput
                label={t('industrialIndex')}
                value={params.industrialIndex}
                onChange={(v) => setParams({ ...params, industrialIndex: v })}
                min={0.5}
                max={2.5}
                step={0.1}
              ...
              {/* Growth Rates */}
              <SliderInput
                label={t('populationGrowth')}
                value={params.populationGrowth}
                onChange={(v) => setParams({ ...params, populationGrowth: v })}
                min={0}
                max={10}
                step={0.5}
                unit="%"
              ...
              <SliderInput
                label={t('industrialSurge')}
                value={params.industrialSurge}
                onChange={(v) => setParams({ ...params, industrialSurge: v })}
                min={0}
                max={20}
                step={1}
                unit="%"
